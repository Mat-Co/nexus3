...
%w(get_cleanup_policy upsert_cleanup_policy delete_cleanup_policy).each do |script_name|
  describe http("#{SCRIPT_API}/#{script_name}") do
...
[nil, '', 'missing-cleanup_policy'].each do |invalid_cleanup_policy|
  describe nexus3_script_http('get_cleanup_policy/run', invalid_cleanup_policy) do
    its('status') { should cmp 200 }
...
describe nexus3_script_http('get_cleanup_policy/run', 'integration_cleanup_policy') do
  its('status') { should cmp 200 }
